function out = output(Network,xk,t,tp,iter,x0,u0,bias_v,fun_v,cen,varargin)
%% INFORMATION
...
[~,QDtilde,~,ADtilde,Btilde,~,~,a,Xreal,~,...
    ~,~,~,~,~,N,n,~,~,~,...
    amatrix,F,Bias,~,~,~,~]=general_init(Network);
xk=xk(1:n*N);
xk=reshape(xk,n,N);
X=[xk,amatrix];
...
out.function_values=[F(x0),fun_v];
